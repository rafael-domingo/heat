...
export default class Default extends React.Component {
    ...
    render() {
        const temperatures = this.props.hourly.map(hourly => <p key={hourly.value} style={{lineHeight: '100px'}}>{hourly.value}</p>)
        const firstTemperature = temperatures[0]
        const remainingTemperatures = temperatures.slice(1)
        const times = this.props.hourly.map(hourly => <p key={hourly.time} style={{lineHeight: '100px'}}>{hourly.time}</p>)
